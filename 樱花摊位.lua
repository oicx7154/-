    ...
    Callback = function(v)
        if v then
            ...
            for _, v in ipairs(Workspace.Item:GetChildren()) do
                ...
                task.wait(Waittime)
                HumanoidRootPart.CFrame = v.Barrel.CFrame + Vector3.new(0,2.5,0)
                task.wait(0.3)
                fireproximityprompt(v.Barrel.ProximityPrompt)
                else
                    return
                        end
                end
...
itemLoot:AddToggle("捡灵魂(idk)",{
    Text = "捡灵魂(idk)",
    Default = false,
    Callback = function(v)
        if v then
            reallll = RunService.ChildAdded:Connect(function()
                for _, v in ipairs(Workspace.Item:GetChildren()) do
                    if v:FindFirstChild("Soul")then
                        task.wait(Waittime)
                        HumanoidRootPart.CFrame = v.CFrame + Vector3.new(0,2.5,0)
                        task.wait(0.3)
                        fireproximityprompt(v.ProximityPrompt)
